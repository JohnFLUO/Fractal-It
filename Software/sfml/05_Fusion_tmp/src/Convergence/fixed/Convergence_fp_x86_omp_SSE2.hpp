...
public:
  ...
  void m128i_show(__m128i v_fixed_val, string varName) {
    ...
    //_mm_storeu_si128(tmp, v_fixed_val);
    for (int i = 0 ; i < 4 ; i++) {
      tmp[i] = _mm_extract_epi32(v_fixed_val, 3-i);
  ...
  void m128i_show64(__m128i v_fixed_val1, __m128i v_fixed_val2, string varName) {
    ...
    //_mm_storeu_si128(tmp, v_fixed_val);
    for (int i = 0 ; i < 4 ; i++) {
      stmp1[i] = _mm_extract_epi32(v_fixed_val1, 3-i);
      stmp2[i] = _mm_extract_epi32(v_fixed_val2, 3-i);
    }
      ...
      for (int y = 0; y < IMAGE_HEIGHT; y++) {
        ...
        for (int x = 0; x < IMAGE_WIDTH;  x+= SSE_CONV_STEP) {
          ...
          for (unsigned int counter = 0; counter < max_iters; counter++) {
            ...
            //v_zImag = _mm_add_epi32(_mm_sll_epi32(v_z_real_z_imag, V_1), v_startImag);
            __m128i v_z_real_z_imag_x2 = _mm_sll_epi32(v_z_real_z_imag, V_1_64);
            //print_fi32_bin(2*z_real_z_imag, "z_real_z_imag_x2 = ");
            //m128i_show(v_z_real_z_imag_x2, "v_z_real_z_imag_x2");

            v_zImag = _mm_add_epi32(v_z_real_z_imag_x2, v_startImag);
            //v_zImag = _mm_add_epi32(_mm_mul_epi32(v_z_real_z_imag, V_2), v_startImag);
#if DEBUG
