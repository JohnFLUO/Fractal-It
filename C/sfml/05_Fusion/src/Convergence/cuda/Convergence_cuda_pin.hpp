...
private:
  ...
  int max_iters;
  cudaStream_t stream;
  unsigned short *host_value_iteration;
  unsigned short *device_value_iteration;
...
public:
  Convergence_CUDA(ColorMap* _colors, int _max_iters){
    ...
    host_value_iteration = (unsigned short *)malloc( sizeof( unsigned short ) * Settings::width*Settings::height );
    device_value_iteration = (unsigned short *)malloc( sizeof( unsigned short ) * Settings::width*Settings::height );
    cudaMallocHost( (void**)&device_value_iteration, Settings::width*Settings::height * sizeof( unsigned short ) );
    //CUDA_MALLOC( (void**)&device_value_iteration, Settting::width*Settting::height * sizeof( unsigned short ) );
...
//    CUDA_MALLOC( (void**)&device_value_iteration, IMAGE_WIDTH*IMAGE_HEIGHT * sizeof( unsigned short ) );
    //cudaMallocHost( (void**)&device_value_iteration, IMAGE_WIDTH*IMAGE_HEIGHT * sizeof( unsigned short ) );


    //---Compute on GPU
    compute(NB_THREADS, zoom, offsetX, offsetY, max_iters,
...
*/
    ...
    cudaMemcpyAsync( host_value_iteration, device_value_iteration, IMAGE_HEIGHT * IMAGE_WIDTH* sizeof( unsigned short ), cudaMemcpyDeviceToHost, stream );
