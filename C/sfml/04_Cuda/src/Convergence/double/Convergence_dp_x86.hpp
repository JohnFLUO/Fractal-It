...
public:
  ...
  //--Initialisation
    ...
    double *host_value = NULL;
    host_value = (double *)malloc( sizeof( double ) * IMAGE_WIDTH*IMAGE_HEIGHT );

    double *device_value = NULL;
    device_value = (double *)malloc( sizeof( double ) * IMAGE_WIDTH*IMAGE_HEIGHT );
    ...
    //---Compute on GPU
    int numThreads = 1024;
    int numBlocks = ( IMAGE_WIDTH + ( numThreads - 1 ) ) / numThreads;

    saxpy(numBlocks, numThreads, zoom, offsetX, offsetY, max_iters,
    ...
    //---retour des valeurs
    CUDA_MEMCPY( host_value, device_value, IMAGE_HEIGHT * IMAGE_WIDTH* sizeof( double ), cudaMemcpyDeviceToHost );
...
/*
    ...
    #pragma omp parallel // on declare une section parallel
    {
      ...
      for (int y = 0; y < IMAGE_HEIGHT; y++) {
        ...
        //real = offsetX - IMAGE_WIDTH / 2.0f * zoom;
        for (int x = 1; x < (IMAGE_WIDTH+1);  x++) {
          real = (offsetX - IMAGE_WIDTH / 2.0f * zoom)+((x)*zoom);
          host_value[y*IMAGE_WIDTH +(x-1)] = process(real, imag, max_iters);
          //real += zoom;
        }
      }
    }
